...
class Kohana_Controller_LogReader extends LogReader_Controller {
    ...
    public function action_index() {
        ...
        $filters = $this->logreader->create_filters(
                $this->request->query('message'), $this->request->query('levels'), $this->request->query('date-from'), $this->request->query('date-to'), $this->request->query('limit')
        );

        $filters_for_autorefresh = $this->logreader->create_filters(
                $this->request->query('message'), $this->request->query('levels'), $this->request->query('date-from'), NULL, $this->request->query('limit')
        );
        ...
        $view->content->messages = $this->logreader->get_messages(
                $filters['date-from'], $filters['date-to'], $filters['limit'], ($current_page - 1) * $filters['limit'], $filters['message']['text'] && $filters['message']['valid'] ? $filters['message']['text'] : NULL, $filters['levels'], array(), NULL
        );
